...
**Обоснование:**
- Использование `for_each` повышает гибкость модуля, упрощает добавление новых подсетей и снижает риск нежелательных изменений Terraform state при модификации инфраструктуры.
...
**Обоснование:**  
- Такой подход позволяет обновлять контент без пересборки Docker-образа, сокращает время деплоя и разделяет ответственность между кодом приложения и его конфигурацией, что соответствует принципам Kubernetes и Twelve-Factor App.
